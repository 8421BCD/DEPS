...
class Contextual(nn.Module):
    ...
    def __init__(
        ...
        self.encoder_docs = Encoder_high(
            ...
            dropout=dropout)
        # 替换rnn作聚合
        self.encoder_aggregation = Encoder_high(
            len_max_seq=args.max_doclen,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=n_layers, n_head=n_head, d_k=d_k, d_v=d_v,
            dropout=dropout)
    ...
    def aggragation_function_token_level(self, doc_fine_grained, docs):
        doc_fine_grained_vec, *_ = self.encoder_aggregation(doc_fine_grained, docs)
        return torch.mean(doc_fine_grained_vec, 1)
    ...
    def forward(self, query, docs1, docs2, features1, features2, long_qdids, longpos, short_qdids, shortpos, docs, docspos, doc1_order, doc2_order):
        ...
        doc1_fine_grained = self.aggragation_function_token_level(doc1_fine_grained, docs1)
        doc2_fine_grained = self.aggragation_function_token_level(doc2_fine_grained, docs2)
        
        # concat the user interest
        doc1_seq_interaction_interest = torch.cat((doc1_fine_grained, qenc_output_5.squeeze(1)), 1)
        doc2_seq_interaction_interest = torch.cat((doc2_fine_grained, qenc_output_5.squeeze(1)), 1)
