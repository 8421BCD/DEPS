'''
    This is an experiment which uses coarse-grained documents interaction(非层次化融入HTPS中session的q和d向量)
'''
import random
import torch
import pickle
...
from model.Model_doc_inter5 import Contextual
import Constants
import torch
...
def prepare_train_data(sat_list, feature_list, doc_list, qids, long_qdids, short_qdids, short_qid_dids):
	if len(doc_list) > 5:
		return
	...
	init_docids = np.zeros((41 + args.max_doc_list_train, args.max_doclen))
	init_docpos = np.zeros(41 + args.max_doc_list_train)
	short_qid_dids_his = short_qid_dids[-40:]
	for i in range(41 + args.max_doc_list_train):
		init_docids[i][0]=1
	for i in range(len(short_qid_dids_his)):
		init_docids[i] = short_qid_dids_his[i]
		init_docpos[i] = i+1
	init_docids[40] = qids
	init_docpos[40] = 41
	for i in range(len(doc_list)):
		init_docids[i + 41] = doc_list[i]
		init_docpos[i + 41] = i + 42
	...
	for i in range(n_targets):
		for j in range(i+1, n_targets):
			...
			if True:
				...
				if delta[i, j]>0:
					doc1_order_train.append(41 + j)
					doc2_order_train.append(41 + i)
				elif delta[i, j]<0:
					doc1_order_train.append(41 + i)
					doc2_order_train.append(41 + j)

def prepare_test_data(short_qdids, long_qdids, qids, feature_list, line_list, doc_list, short_qid_dids):
	if len(doc_list) != 50:
		return
	for docid in range(len(doc_list)):
		...
		# update
		init_docids = np.zeros((41 + args.max_doc_list_test, args.max_doclen))
		init_docpos = np.zeros(41 + args.max_doc_list_test)
		short_qid_dids_his = short_qid_dids[-40:]
		for i in range(41 + args.max_doc_list_test):
			init_docids[i][0]=1
		for i in range(len(short_qid_dids_his)):
			init_docids[i] = short_qid_dids_his[i]
			init_docpos[i] = i+1
		...
		for line in fhand:
			...
			if queryid != last_queryid:
				if last_querytime >= '2006-04-03 00:00:00' and last_querytime <= '2006-05-16 00:00:00' and key == 1: # training data
					prepare_train_data(sat_list, feature_list, doc_list, last_qids, long_qdids, short_qdids, short_qid_dids)
					key = 0
				elif last_querytime > '2006-05-16 00:00:00':
					prepare_test_data(short_qdids, long_qdids, last_qids, feature_list, line_list, doc_list, short_qid_dids)
				short_qid_dids += last_qid_dids
				last_qid_dids = [qids]
