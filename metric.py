...
class Metric(object):
	...
	def __init__(self):
		...
		with open('../../data/entropy.dict', 'rb') as fr:
	...
	def evaluate(self, filehandle, result_path, epoch, logger):
		"""
		evaluate queries in the filehadle, each line is the same as the training dataset.
		"""
		last_queryid = 0
		f = open('result/' + self.__class__.__name__+'.txt', 'w')
		re = open(result_path,'a')
		...
		re.write("epoch=" + str(epoch) + ":The "+self.__class__.__name__+" of new ranking is {}.\n".format(mscore2/nquery))
		logger.info("epoch=" + str(epoch) + ":The "+self.__class__.__name__+" of new ranking is {}.".format(mscore2/nquery))
