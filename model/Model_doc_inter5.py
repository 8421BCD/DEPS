...
class Contextual(nn.Module):
    ...
    def __init__(
        ...
        self.encoder_docs_query = Encoder_high(
            len_max_seq=50+args.max_doc_list_test,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=n_layers, n_head=n_head, d_k=d_k, d_v=d_v,
            dropout=dropout)
    ...
    def forward(self, query, docs1, docs2, features1, features2, long_qdids, longpos, short_qdids, shortpos, docs, docspos, doc1_order, doc2_order):
        ...
        #     docsenc_output1[:, i], *_ = self.encoder_docs(docsenc_output1[:, i], docs[:, i])
        docs_doc2vec = torch.mean(docsenc_output1, -2)
        # query_docs_vec = torch.cat([docs_doc2vec, qenc_output_4, qenc_output_5], 1)
        query_docs_output, *_ = self.encoder_docs_query(docs_doc2vec, docspos, needpos=True)
