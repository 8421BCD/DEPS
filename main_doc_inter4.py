'''
    This is an experiment which uses coarse-grained documents interaction(层次化融入HTPS中session的qd向量)
'''
import random
import torch
import pickle
...
from model.Model_doc_inter4 import Contextual
import Constants
import torch
...
def prepare_train_data(sat_list, feature_list, doc_list, qids, long_qdids, short_qdids):
	...
	init_session_docs_pos_train = np.zeros((args.max_sessionlen + 2))
	init_session_docs_pos_train[-2] = args.max_sessionlen + 1
	init_session_docs_pos_train[-1] = args.max_sessionlen + 2
	for i in range(len(short_qdids_his)):
		init_session_docs_pos_train[i] = i + 1
	...
	for i in range(n_targets):
		for j in range(i+1, n_targets):
			...
			if True:
				...
				session_docs_pos_train.append(init_session_docs_pos_train)
...
def prepare_test_data(short_qdids, long_qdids, qids, feature_list, line_list, doc_list):
	...
	for docid in range(len(doc_list)):
		...
		init_session_docs_pos_test = np.zeros((args.max_sessionlen + 2))
		init_session_docs_pos_test[-2] = args.max_sessionlen + 1
		init_session_docs_pos_test[-1] = args.max_sessionlen + 2
		for i in range(len(short_qdids_his)):
			init_session_docs_pos_test[i] = i + 1

		docs_list_test.append(init_docids)
		docs_pos_test.append(init_docpos)
		session_docs_pos_test.append(init_session_docs_pos_test)
...
def train(train_loader):
	...
	avg_loss = 0
	model.train()
	for batch_idx, (query, docs1, docs2, label, next_q, features1, features2, delta, long_qdids, longpos, short_qdids, shortpos, docs, docspos, doc1_order, doc2_order, session_docs_pos) in enumerate(tqdm_train):
		optimizer.zero_grad()
		...
		session_docs_pos = session_docs_pos.cuda(args.cudaid)

		score, pre, p_score = model(query, docs1, docs2, features1, features2, long_qdids, longpos, short_qdids, shortpos, docs, docspos, doc1_order, doc2_order, session_docs_pos)
...
def score(test_loader, load=False):
	...
	for batch_idx, (query, docs, features, long_qdids, longpos, short_qdids, shortpos, lines, docslist, docspos, doc_order, session_docs_pos) in enumerate(tqdm_test):
		...
		score, pre, p_score = model(query, docs, docs, features, features, long_qdids, longpos, short_qdids, shortpos, docslist, docspos, doc_order, doc_order, session_docs_pos)
...
if __name__ == '__main__':
	...
	train_loader = torch.utils.data.DataLoader(
		Dataset_train(
			...
			session_docs_pos=session_docs_pos_train),
	...
	score_loader = torch.utils.data.DataLoader(
		Dataset_score(
			...
			session_docs_pos=session_docs_pos_test),
		batch_size=args.batch_size,
