...
class Contextual(nn.Module):
    ...
    def __init__(
        ...
        self.encoder_docs_interest = Encoder_high(
            len_max_seq=args.max_doc_list_test+5,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=n_layers, n_head=n_head, d_k=d_k, d_v=d_v,
            dropout=dropout)

        self.encoder_session_doc = Encoder_high(
            len_max_seq=args.max_sessionlen+5,
            d_word_vec=d_word_vec, d_model=d_model, d_inner=d_inner,
            n_layers=n_layers, n_head=n_head, d_k=d_k, d_v=d_v,
            dropout=dropout)

        # get the scores of all the documents
        self.score_7_layer = nn.Sequential(nn.Linear(100, 1),nn.Tanh())
        self.score_8_layer = nn.Sequential(nn.Linear(100, 1),nn.Tanh())
        ...
        self.score_layer = nn.Linear(8, 1)
    ...
    def forward(self, query, docs1, docs2, features1, features2, long_qdids, longpos, short_qdids, shortpos, docs, docspos, doc1_order, doc2_order, session_docs_pos):
        ...
        docsenc_output1 = self.embedding(docs)
        # for i in range(docsenc_output1.shape[1]):
        #     docsenc_output1[:, i], *_ = self.encoder_query(docsenc_output1[:, i], docs[:, i])
        docs_doc2vec = torch.mean(docsenc_output1, -2)
        docs_interest = torch.cat([docs_doc2vec, qenc_output_4, qenc_output_5], 1)
        docs_interest_output, *_ = self.encoder_docs_interest(docs_interest, docspos)
        docs_interacted_doc1, docs_interacted_doc2 = [], []
        for i in range(docs_interest_output.shape[0]):
            docs_interacted_doc1.append(docs_interest_output[i, doc1_order[i]])
            docs_interacted_doc2.append(docs_interest_output[i, doc2_order[i]])
        docs_interacted_doc1 = torch.stack(docs_interacted_doc1)
        docs_interacted_doc2 = torch.stack(docs_interacted_doc2)
        score_1_7 = self.score_7_layer(docs_interacted_doc1)
        score_2_7 = self.score_7_layer(docs_interacted_doc2)
        session_doc1 = torch.cat([sq_qdenc_copy, docs_interacted_doc1.unsqueeze(1)], 1)
        session_doc2 = torch.cat([sq_qdenc_copy, docs_interacted_doc2.unsqueeze(1)], 1)
        session_doc1_output, *_ = self.encoder_session_doc(session_doc1, session_docs_pos, needpos=True)
        session_doc2_output, *_ = self.encoder_session_doc(session_doc2, session_docs_pos, needpos=True)
        score_1_8 = self.score_8_layer(session_doc1_output[:, -1])
        score_2_8 = self.score_8_layer(session_doc2_output[:, -1])
        ...
        score1_all = torch.cat([score_1_1, score_1_2, score_1_3, score_1_4, score_1_5, score_1_6, score_1_7, score_1_8], 1)
        score2_all = torch.cat([score_2_1, score_2_2, score_2_3, score_2_4, score_2_5, score_2_6, score_2_7, score_2_8], 1)
